...
5. For the last mod [Better Respawn](https://github.com/xieleo5/better-respawn/tree/1.19), you need to manually clone and compile.
   ...
   * After you clone the repo, you need to `cd better-respawn`, and `git checkout 1.19`, then remove the `'forge'` string in the last line of `settings.gradle`. Then run `chmod +x gradlew`, then `./gradlew build` to compile the mod. You will find the compiled jar file in `better-respawn/fabric/build/libs/better-respawn-fabric-1.19-2.0.0.jar`. Put the jar file to the mod folder.
